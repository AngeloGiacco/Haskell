...
update new@(id,val) (curr@(name,_):vars)
  | name == id = (name,val) : vars
...
eval (Apply o x y) s = apply o (eval x s) (eval y s)
eval (Phi a b) _     = undefined
...
scan id n (Assign x y:ss)
  | isConstAssignment res = let (Assign _ (Const n')) = res in ((x,n'):w, b)
  | otherwise             = (w,res:b)  
  where 
    res = Assign x (sub id n y)
    (w,b) = scan id n ss 
scan id n (If e b1 b2:ss) = (w ++ w' ++ w'' , If (sub id n e) b1' b2':bs) 
  where 
    (w',b1')  = scan id n b1
    (w'',b2') = scan id n b2
    (w,bs)    = scan id n ss
scan id n (DoWhile b e:ss) = (w'++w,DoWhile b' (sub id n e):bs)
  where
    (w',b') = scan id n b 
    (w,bs)  = scan id n ss
scan id n [] = ([],[])
...
propagateConstants = propagateConstants' [("$INVALID", 0)]
  where 
    propagateConstants' :: Worklist -> Block -> Block
    propagateConstants' ((id,n):ws) b = propagateConstants' (w' ++ ws) res
      where 
        (w',res) = scan id n b
...
insertVars :: Statement -> Statement -> Statement
insertVars (If e b1 b2) (Assign id (Phi e1 e2)) = If e b1' b2'
  where 
    b1' = b1 ++ [Assign id e1]
    b2' = b2 ++ [Assign id e2]
